...
import { Brain, Plus, Pencil, Trash2, Check, Cpu, Loader2, Share2, MessageSquare, Wrench } from "lucide-react";
import {
  getAgent,
  getConfig,
  listProviders,
...
export default function AgentModelsPage() {
  ...
  // promptMode "" means "no override saved" — runtime falls back to
  // "agent". The select renders "" as the "Default (agent)" choice so
  // a fresh agent shows a sensible option without auto-creating a
  // configs row on first render.
  const [promptMode, setPromptMode] = useState<"" | "agent" | "chatbot" | "minimal">("");
  // toolAllowlistText is the comma-separated display form the user
  // sees and types into. We persist as []string on the server side;
  // the conversion happens at save time. Empty string = clear override
  // (agent sees all registered tools at runtime).
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      // promptMode / toolAllowlist may be undefined on a fresh agent
      // (no row yet) or on a backend that predates the field — both
      // collapse to "no override" in the UI.
  ...
  // PromptMode change: optimistic update, send the new mode (or "" to
  // clear the override and fall back to runtime default "agent").
  ...
  // Tool allowlist save on blur — the user types into a comma-separated
  // input, we split & trim here, then send as string[]. Empty string =
  // clear override.
  const handleToolAllowlistBlur = async () => {
    ...
    try {
      ...
      // Re-normalize the displayed text to match what we just saved —
      // collapses double commas, strips whitespace — so the input
      // doesn't lie about what's stored.
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      ...
      <div className="rounded-lg border border-border bg-card p-5">
        ...
        <p className="text-xs text-muted-foreground mt-2">
          <strong>Agent</strong> emits the full framework prompt (task delegation,
          tool-use discipline, workspace self-update, scheduling).{" "}
          <strong>Chatbot</strong> drops those agent-loop sections so persona
          files (SOUL.md / IDENTITY.md) shape voice directly — recommended for
          companion / role-play / customer-support bots.{" "}
          <strong>Minimal</strong> emits only the date + bootstrap files; you
          take full responsibility for what the LLM sees.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-border bg-card p-5">
        ...
        <p className="text-xs text-muted-foreground mt-2">
          ...
          <code className="mx-1 text-[11px]">exec</code>/
          <code className="text-[11px]">web_fetch</code>/etc. from chatbot
          agents that should only call messaging tools. Names that don&apos;t
          match a registered tool are silently dropped at request time; if
          nothing in the list matches, the LLM sees no tools (fail-closed).
        </p>
      </div>
